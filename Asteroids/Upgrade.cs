...
namespace Asteroid
{
    class Upgrade : Sprite
    {
        ...
        public Upgrade(Vector2 position, Type type, TimeSpan time, TimeSpan duration, TimeSpan shotsTime, Texture2D[] damage, Bullet shot, int rarity, Texture2D image, float rot, float scale, Color color) : base(position, image, rot, scale, color)
        {
            Position = position;
            leType = type;
            DespawnTime = time;
